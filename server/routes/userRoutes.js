...
const {
  generateUserToken,
  verifyUserToken,
} = require("../modules/JWT/userTokens");

const {
  sendVerificationEmail,
  sendForgotPasswordEmail,
  sendChangedPasswordEmail,
} = require("../modules/Nodemailer/sendTemplates");
...
router.post("/validate", (req, res) => {
  // RECEIVES USER id AND THE TOKEN AN VERIFY THE TOKEN, IF VALIDATES, EXTRACTS THE ID FROM THE TOKEN.
  ...
  if (req.body && req.body.id && req.body.token) {
    ...
    crudUsers
      ...
      .then((user) => {
        ...
        if (!obj.isValid) {
          // IF THE TOKEN IS NOT VALID ITS BECAUSE IT REACHED THE LIMIT DURATION TIME, SO WE HAVE TO GENERATE A NEW TOKEN AND RESEND..
        ...
        if (id === obj.id) {
          // IF THE TOKEN IS VALID AND ID === ID IN THE TOKEN...CONTINUES
