...
const {
  uploadImageFiles: uploadCategoryImages,
} = require("../middleware/multerCategoryImageFiles");

const {
  uploadImageFiles: uploadSubcategoryImages,
} = require("../middleware/multerSubcategoryImageFiles");
...
router.get("/admin", paginate.middleware(10, 50), (req, res) => {
  ...
  crudCategories
    .getAllCategoriesAdmin(filter, page, limit, sort)
    .then((results) => {
...
router.get("/sub/admin", paginate.middleware(10, 50), (req, res) => {
  ...
  crudSubcategories
    .getAllSubcategoriesAdmin(filter, page, limit, sort)
    .then((results) => {
      /* result is an Object with the following properties:
...
router.put("/:categoryId", secureAdmin(), (req, res) => {
  if (req.params.categoryId && req.body && req.body.filter) {
    ...
    crudCategories
      .updateCategory(categoryId, filter)
      .then((category) => {
        res.status(200).json({
          error: null,
          ok: true,
...
router.post("/:categoryId/sub", secureAdmin(), (req, res) => {
  if (req.params.categoryId && req.body && req.body.filter) {
    ...
    crudSubcategories
      .addSubcategoryWithFilter(categoryId, filter)
      .then((subcategory) => {
        res.status(200).json({
          error: null,
          ok: true,
...
router.put("/sub/:subcategoryId", secureAdmin(), (req, res) => {
  if (req.params.subcategoryId && req.body && req.body.filter) {
    ...
    crudSubcategories
      .updateSubcategory(subcategoryId, filter)
      .then((subcategory) => {
        res.status(200).json({
          error: null,
          ok: true,
...
router.post(
  "/:categoryId/images/all",
  secureAdmin(),
  uploadCategoryImages.array(
    "images",
    config.app.items.CATEGORIES_IMAGES_MAX_COUNT
  ),
  (req, res) => {
...
router.post(
  "/sub/:subcategoryId/images/all",
  secureAdmin(),
  uploadSubcategoryImages.array(
    "images",
    config.app.items.SUBCATEGORIES_IMAGES_MAX_COUNT
  ),
  (req, res) => {
    // RECEIVES IMAGES FROM MULTER IN FORM DATA FORMAT AND SAVE THEM IN GRIDFS
    // uploadImageFiles SAVES THE FILES ON GFS AND SAVES THE ID ON images FIELD IN THE ITEM DOCUMENT
...
router.post(
  "/sub/:subcategoryId/images/one",
  secureAdmin(),
  uploadSubcategoryImages.single("image"),
  (req, res) => {
...
router.delete("/sub/many", secureAdmin(), (req, res) => {
  ...
  if (filter.ids.length > 0) {
    crudSubcategories
      .deleteManySubCategories(filter)
      .then(() => {
        res.status(200).json({
          error: null,
          ok: true,
...
router.delete(
  "/sub/:subcategoryId/images/one/:imageId",
  secureAdmin(),
  (req, res) => {
    ...
    if (req.params.subcategoryId && req.params.imageId) {
      ...
      crudSubcategories
        .getSubcategoryById(subcategoryId)
        .then((item) => {
          return crudSubcategories.deleteSubcategoryImage(
            subcategoryId,
            imageId
          );
        })
...
router.delete("/sub/:subcategoryId/images/many", (req, res) => {
  ...
  if (req.params.subcategoryId) {
    ...
    if (filter.ids.length > 0) {
      crudSubcategories
        .deleteManySubcategoryImages(subcategoryId, filter)
        .then(() => {
          res.json({
            error: null,
            ok: true,
...
router.delete("/:categoryId/images/many", (req, res) => {
  ...
  if (req.params.categoryId) {
    ...
    if (filter.ids.length > 0) {
      crudCategories
        .deleteManyCategoryImages(categoryId, filter)
        .then(() => {
          res.json({
            error: null,
            ok: true,
