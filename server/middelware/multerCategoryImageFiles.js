...
const gfsStorageImageFiles = new GridFSStorage({
  ...
  file: function (req, file) {
    const {
      categoryId,
    } = req.params; /* EVERY TIME WE USE UPLOAD MIDDLEWARE WE HAVE TO RECEIVE THE itemId PARAMETER FROM THE REQUEST, SO THAT generateFileId Function CAN SAVE THE imageId IN THE USERS COLLECTION. */
    ...
    return new Promise((resolve, reject) => {
      crudCategories
        ...
        .then((item) => {
          ...
          if (
            item.images.length < config.app.items.CATEGORIES_IMAGES_MAX_COUNT
          ) {
            if (
