...
router.get("/admin", paginate.middleware(10, 50), (req, res) => {
  ...
  crudItems
    .getItemsAdmin(filter, page, limit, sort)
    .then((results) => {
...
router.get("/", paginate.middleware(20, 50), (req, res) => {
  ...
  crudItems
    .getItems(filter, page, limit, sort)
    .then((results) => {
      /* result is an Object with the following properties:
...
router.post(
  "/:itemId/images/all",
  secureAdmin(),
  uploadImageFiles.array("images", config.app.items.ITEMS_IMAGES_MAX_COUNT),
  (req, res) => {
...
router.delete("/:itemId/images/many", (req, res) => {
  ...
  if (req.params.itemId) {
    ...
    if (filter.ids.length > 0) {
      crudItems
        .deleteManyItemImages(itemId, filter)
        .then((item) => {
          res.json({
            error: null,
            ok: true,
