...
const schema = yup.object().shape({
  password: yup
    ...
    .matches(
      ...
      "Password is not valid (see notes below)"
    )
...
const ErrorAlert = ({ msg }) => (
  <CSSTransition appear in timeout={500} classNames="signup-form-error-effect">
    <div id="signup-form-error-container">
      {errorIcon}
      <span id="signup-form-error-message">{msg}</span>
    </div>
  </CSSTransition>
);
...
const handleFormSubmit = (
  ...
  const NotificationContent = (
    <>
      Your password has been successfully reset. Please log back into BitZone
      website using your new password.
    </>
  );
  ...
  fetchPostResetPasswordUser(id, token, password)
    .then((response) => {
      if (response.ok) {
...
const eyeIcon = (visible, handleClick) => {
  if (visible) {
    ...
    return (
      <button
        type="button"
        className="resetpsw-form-eye-button"
        onClick={handleClick}
      >
        ...
        <div id="resetpsw-form-container">
          <div id="resetpsw-form">
              ...
              {({ values, errors, isSubmitting, submitForm }) => (
                <Form method="get" action="form">
                  {Object.keys(errors).length > 0 ? (
                    <ErrorAlert msg={errors[Object.keys(errors)[0]]} />
                  ) : null}
                  <Field name="password">
                    {({ field }) => (
                      <div className="resetpsw-form-text-div">
                        <p>
                          <label htmlFor="resetpsw-password">
                            New Password :
                          </label>
                          {eyeIcon(pswVisible, handleEyeClick)}
                        </p>
                        <input
                          {...field}
                          autoComplete="off"
                          type={psw1Type}
                          className="form-effect-border-bottom"
                          id="resetpsw-password"
                          autoFocus
                          ref={inputRef1}
                        />
                  ...
                  <Field name="confirmPassword">
                    {({ field }) => (
                      <div className="resetpsw-form-text-div">
                        <p>
                          <label htmlFor="resetpsw-confirm-password">
            ...
            <div id="resetpsw-form-footer">
              <p>
                Please set a new 8-20 character password. Your password must{" "}
                <strong>
                  start with a letter (a-z / A-Z) and contain at least one
                  number (0-9).
                </strong>{" "}
                Permitted symbols: dot (.), underscore (_) and dash (-).
              </p>
            </div>
          </div>
        </div>
