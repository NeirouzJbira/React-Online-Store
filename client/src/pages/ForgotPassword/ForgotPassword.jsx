...
const handleFormSubmit = (
  ...
  const NotificationContent = (
    <>
      ...
      <span className="content-different">{username}</span> so that you can
      reset your password. Please check your email inbox for instructions on how
      to proceed.
    </>
  );

  fetchPostForgotPasswordUser(username)
    .then((response) => {
      if (response.ok) {
...
const ErrorAlert = ({ msg }) => (
  <CSSTransition
    ...
    in
    timeout={500}
    classNames="forgotpsw-form-error-effect"
  >
    <div id="forgotpsw-form-error-container">
      {errorIcon}
      <span id="forgotpsw-form-error-message">{msg}</span>
    </div>
  </CSSTransition>
);
                  ...
                  <Field name="username">
                    {({ field }) => (
                      <div className="forgotpsw-form-text-div">
                        <p>
                          <label htmlFor="forgotpsw-username">Email :</label>
                        </p>
                        <input
                          {...field}
                          autoComplete="off"
