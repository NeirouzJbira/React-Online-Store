...
import {
  fetchGetLoginUser,
  fetchGetLogoutUser,
} from "../../../modules/fetchFunctions/users";
    ...
    fetchGetLogoutUser()
      ...
      .catch((err) => {
        this.setState({
          error: true,
          errorMessage:
            getErrorMessage("009") + err.message + getErrorMessage("010"),
        });
      });
  }
    ...
    const ErrorAlert = ({ msg }) => (
      <CSSTransition appear in timeout={300} classNames="header-error-effect">
        <div id="header-error-container">
          {errorIcon}
          <span id="header-error-message">{msg}</span>
        </div>
      </CSSTransition>
    );
    ...
    return (
      <header id="header-container">
        ...
        >
          <FontAwesomeIcon
            id="icon-departments"
            icon={this.props.isDepartmentsOpen ? "caret-up" : "caret-down"}
            size="lg"
          />
          Departments
        </button>
        <button
          type="button"
          id="mobile-btnDepartments"
          onClick={this.handleDepartments}
        >
          <FontAwesomeIcon
            id="mobile-icon-departments"
            icon="align-justify"
            size="lg"
          />
        </button>
        ...
        <div id="header-user-nav">{isLoggedUser ? <UserNav /> : null}</div>
        <div id="header-cart">
          <Cart
            size="lg"
            value={isLoggedUser ? cartCount : null}
            fgColor={this.props.theme.colorYellowDark}
            bgColor={this.props.theme.colorBlueBase}
            counterColor="white"
            pxChangeLargeToSmall="680px"
        ...
        {!isLoggedUser ? (
          <div id="header-new-user">
            <p id="new-to-bitzone">
              <Badge variant="warning">New</Badge> to BitZone?{" "}
              <span id="signup-container">
                <Link to="/signup">
                  <FontAwesomeIcon
                    icon="user-shield"
                    className="icon-main-nav icon-signup"
                  />
                  Sign Up
                </Link>
              </span>
            ...
            <p id="login-container">
              <Link to="/login">
                <FontAwesomeIcon
                  icon="sign-in-alt"
                  className="icon-main-nav icon-login"
                />
                Login
              </Link>
            </p>
        ...
        {isLoggedUser ? (
          <div id="header-logged-user">
            <Avatar
              arrowColor="white"
              textColor={this.props.theme.colorBlueBase}
              value={userProfile.firstname}
              borderColor="white"
              menuColor={this.props.theme.colorBg1}
        ...
        <div id="header-mobile-logged-user">
          {isLoggedUser ? (
            <Avatar
              small
              avatarColor={this.props.theme.colorPurpleClear}
              arrowColor="white"
              textColor={this.props.theme.colorBlueBase}
              value={userProfile.firstname}
              borderColor="white"
              menuColor={this.props.theme.colorLightBlue}
              handleLogout={this.handleLogout}
            />
          ) : null}
        </div>
