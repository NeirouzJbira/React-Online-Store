...
function SubdepList({
  ...
  const subdepartmentLine = (id, name, handleClick) => {
    return (
      <li className="departments-small-subdeplist-line" key={id}>
        <a
          className="departments-small-subdeplist-line-link"
          href="#"
          onClick={handleClick}
        >
          {name}
  ...
  return (
    <CSSTransition
      in={visible}
      timeout={500}
      classNames="my-mobile-menu-subdeplist-effect"
    >
      <div id="departments-small-subdeplist-container">
        <h4>{selectedDepartment.name}</h4>
        <button
          type="button"
          id="departments-small-subdeplist-back-button"
          onClick={handleBackButtonClick}
        >
          <FontAwesomeIcon
            className="departments-small-subdeplist-back-button-icon"
            icon="arrow-left"
            size="lg"
          />
          Back
        ...
        <ul id="departments-small-subdeplist">
          {subDepartmentsList.map((item) =>
            subdepartmentLine(item._id, item.name, (e) =>
              handleMouseClick(item._id, e)
            )
          )}
        </ul>
